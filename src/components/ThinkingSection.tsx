        ...
        <div className="space-y-0">
          {items.map((item, i) => (
            <motion.div
              ...
              className="grid lg:grid-cols-[1fr_1fr] gap-6 lg:gap-10 py-6 border-b border-border group"
            >
              ...
              <p className="text-base lg:text-lg text-muted-foreground leading-relaxed self-center text-white">
                <span className="font-normal text-white">{item.text.split(". ")[0]}.</span> {item.text.split(". ").slice(1).join(". ")}
              </p>
            </motion.div>
          ))}
        </div>
